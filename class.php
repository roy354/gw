...
class social {
	...
	function login($user, $pass) {
		...
		curl_setopt($ch, CURLOPT_COOKIEJAR, $user);
		...
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		curl_setopt($ch, CURLOPT_COOKIEFILE, $this->user);
		...
		curl_setopt($ch, CURLOPT_COOKIEJAR, $this->user);
		curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
		curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
		$hasil = curl_exec($ch);
		...
		//var_dump($token);
		fwrite(fopen("token.txt", "w+"), $token[1][0]);
	}
	...
	function send_balance($jumlah) {
		...
		$headers[] = 'Content-Type: application/x-www-form-urlencoded; charset=UTF-8';
		$headers[] = 'Accept: */*';
		$headers[] = 'Referer: https://www.askdaraz.com/wallet/';
		$headers[] = 'X-Requested-With: XMLHttpRequest';
		$headers[] = 'Connection: keep-alive';
		$headers[] = 'Origin: https://www.askdaraz.com';
		curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
	...
	function cari($w) {
		...
		curl_setopt($ch, CURLOPT_URL, 'https://www.askdaraz.com/requests.php?hash='.$this->token.'&f=posts&s=load_more_posts&filter_by_more=all&after_post_id=' . $w . '&user_id=0&page_id=0&group_id=0&event_id=0&posts_count=44&is_api=0&ad_id=0&story_id=0&_=1566014392392');
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
